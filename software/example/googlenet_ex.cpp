...
int main(int argc, char **argv)
{
	...
	//# Get Path
	char *dirpath    = "/mnt/models/GoogleNet"; 	/* Path to the model directory */
	char *prototxt   = "deploy.prototxt";			/* Prototxt file name residing in model directory */
	char *caffemodel = "GoogleNet.caffemodel";		/* caffemodel file name residing in model directory */
	char *mean_file	 = "mean_file.txt";				/* mean file name residing in model directory */
	int  mean_type	 = 0;							/* 0 for .txt, 1 for .binaryproto */

	char *img_path  = "/mnt/models/GoogleNet/input/camel.jpg";	/* Correct the path accordingly */
	...
	FILE *out = fopen("/mnt/models/GoogleNet/out/out.txt", "w");
