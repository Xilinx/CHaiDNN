...
{
	if(argc < 5) {
		std::cout << "Usage : ./googlenet_ex.elf <quant_mode> <bit_width> <img1_path> <img2_path>" << std::endl;
		std::cout << "quant_mode : Xilinx / DynamicFixed" << std::endl;		
	...
	if(quant_mode_arg == "Xilinx" && bit_width_arg == 6) {
		...
		prototxt   = "GoogleNetNoLRN_6Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 0;
	}
	// Xilinx quantmode 8 bit
	else if(quant_mode_arg == "Xilinx" && bit_width_arg == 8) {
		dirpath    = "models/GoogleNetWithoutLRN/8Bit";
		caffemodel = "GoogleNetNoLRN_8Bit_CHaiDNN.caffemodel";
		prototxt   = "GoogleNetNoLRN_8Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 0;
	}
	// DynamicFixed quantmode 6 bit
	else if(quant_mode_arg == "DynamicFixed" && bit_width_arg == 6) {
		dirpath    = "models/GoogleNetWithoutLRN/6Bit";
		caffemodel = "GoogleNetNoLRN_6Bit_CHaiDNN.caffemodel";
		prototxt   = "GoogleNetNoLRN_6Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 1;
	}
	// DynamicFixed quantmode 8 bit
	else if(quant_mode_arg == "DynamicFixed" && bit_width_arg == 8) {
		dirpath    = "models/GoogleNetWithoutLRN/8Bit";
		caffemodel = "GoogleNetNoLRN_8Bit_CHaiDNN.caffemodel";
		prototxt   = "GoogleNetNoLRN_8Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 1;
		...
		std::cout << "quant_mode argument should be either Xilinx or DynamicFixed" << std::endl;
		std::cout << "Usage : ./googlenet_ex.elf <quant_mode> <bit_width> <img1_path> <img2_path>" << std::endl;
		std::cout << "quant_mode : Xilinx / DynamicFixed" << std::endl;		
		std::cout << "bit_width : 8 / 6" << std::endl;
		std::cout << "img1_path : Path to the first image" << std::endl;
