...
{
	if(argc < 5) {
		std::cout << "Usage : ./vgg16_ex.elf <quant_mode> <bit_width> <img1_path> <img2_path>" << std::endl;
	...
	if(quant_mode_arg == "Xilinx" && bit_width_arg == 6) {
		dirpath    = "models/VGG-16/6Bit";
		caffemodel = "VGG16_6Bit_CHaiDNN.caffemodel";
		prototxt   = "VGG16_6Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 0;
	}
	// Xilinx quantmode 8 bit
	else if(quant_mode_arg == "Xilinx" && bit_width_arg == 8) {
		dirpath    = "models/VGG-16/8Bit";
		caffemodel = "VGG16_8Bit_CHaiDNN.caffemodel";
		prototxt   = "VGG16_8Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 0;
	}
	else {
