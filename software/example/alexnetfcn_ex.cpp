...
int main(int argc, char **argv)
{
	...
	void *input = malloc(500*500*3*sizeof(char));
	//# Create memory with max size of output layer
	void *output = malloc(500*500*sizeof(int));
	
	//# Get Path
	char *dirpath    = "/mnt/models/AlexNetFCN"; 	/* Path to the model directory */
	char *prototxt   = "deploy.prototxt";			/* Prototxt file name residing in model directory */
	char *caffemodel = "AlexNetFCN.caffemodel";		/* caffemodel file name residing in model directory */
	char *mean_file	 = "mean_file.txt";				/* mean file name residing in model directory */
	int  mean_type	 = 0;							/* 0 for .txt, 1 for .binaryproto */

	char *img_path  = "/mnt/models/AlexNetFCN/input/input.jpg";	/* Correct the path accordingly */
	
	//# Call Initialization: One time initalization of network params
	xiInit(dirpath, prototxt, caffemodel, mean_file, mean_type, jobQueue, ptrsList, 500, 500);
	...
	loadImagetoBuffptr(img_path, (unsigned char*)input, 500, 500, 3, 500, 500);
	...
	FILE *out = fopen("/mnt/models/AlexNetFCN/out/out.txt", "w");
	...
	for(int i = 0; i < 500; ++i)
	{
		for(int j = 0; j < 500; ++j)
		{
			fprintf(out, "%d ", ((int*)output)[i*500+j]);
		}
		fprintf(out, "\n");
	}
	fclose(out);
