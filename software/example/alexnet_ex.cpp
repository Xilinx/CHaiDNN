...
{
	if(argc < 5) {
		std::cout << "Usage : ./alexnet_ex.elf <quant_mode> <bit_width> <img1_path> <img2_path>" << std::endl;
	...
	if(quant_mode_arg == "Xilinx" && bit_width_arg == 6) {
		dirpath    = "models/AlexNet/6Bit";
		caffemodel = "AlexNetNoLRN_6Bit_CHaiDNN.caffemodel";
		prototxt   = "AlexNetNoLRN_6Bit_deploy_CHaiDNN.prototxt";
        inp_mode   = 0;
	}
	// Xilinx quantmode 8 bit
	else if(quant_mode_arg == "Xilinx" && bit_width_arg == 8) {
		dirpath    = "models/AlexNet/8Bit";
		caffemodel = "AlexNet_8b_io_6b_wts_deploy_CHaiDNN.caffemodel";
		prototxt   = "AlexNet_8Bit_deploy_CHaiDNN.prototxt";	
        inp_mode   = 0;
	}
	else {
